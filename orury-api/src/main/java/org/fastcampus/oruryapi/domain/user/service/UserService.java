...
public class UserService {
    ...
    public ResponseMypage readMypage(RequestId requestId){
        User user = userRepository.findById(requestId.id()).orElseThrow(()->new BusinessException(UserErrorCode.USER_NOT_FOUND));
        return ResponseMypage.toDto(UserDto.from(user));
    }

    public void updateProfileImage(RequestProfileImage requestProfileImage){
        User user = userRepository.findById(requestProfileImage.id()).orElseThrow(()->new BusinessException(UserErrorCode.USER_NOT_FOUND));
    ...
    public void updateUserInfo(RequestUserInfo requestUserInfo){
        User user = userRepository.findById(requestUserInfo.id()).orElseThrow(()-> new BusinessException(UserErrorCode.USER_NOT_FOUND));
